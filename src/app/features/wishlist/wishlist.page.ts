...
@Component({
  ...
  template: `
    <section class="theme-container" style="padding:16px 0;">
      <h2 class="primary-text" style="margin:0 0 12px;">Wishlist</h2>
      <div *ngIf="lists.wishlist().length; else empty">
        <div class="grid">
          <div class="card" *ngFor="let p of lists.wishlist()">
            ...
            <div class="meta">
              ...
              <div class="row between">
                ...
                <button class="outline" (click)="lists.removeFromWishlist(p.id)">Quitar</button>
              </div>
            </div>
          </div>
        </div>
        <div class="row end" style="margin-top:12px;">
          <button class="outline" (click)="lists.clearWishlist()">Limpiar wishlist</button>
        </div>
      </div>
      <ng-template #empty>
        <div class="text-muted">No tienes productos en tu wishlist.</div>
      </ng-template>
    </section>
  `,
  styles: [`
